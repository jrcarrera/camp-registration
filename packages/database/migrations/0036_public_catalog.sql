ALTER TABLE organizations
  ...
  ADD COLUMN public_tagline text NULL CHECK (public_tagline IS NULL OR (length(btrim(public_tagline)) BETWEEN 1 AND 120)),
  ADD COLUMN public_description text NULL CHECK (public_description IS NULL OR (length(btrim(public_description)) BETWEEN 1 AND 1500)),
  ADD COLUMN brand_primary_color text NOT NULL DEFAULT '#166534' CHECK (brand_primary_color ~ '^#[0-9A-Fa-f]{6}$'),
  ADD COLUMN brand_logo_url text NULL CHECK (brand_logo_url IS NULL OR length(brand_logo_url) <= 2048),
  ADD COLUMN public_website_url text NULL CHECK (public_website_url IS NULL OR length(public_website_url) <= 2048),
  ADD COLUMN public_contact_email text NULL CHECK (public_contact_email IS NULL OR length(public_contact_email) <= 320);

GRANT UPDATE (
  public_catalog_enabled, public_tagline, public_description, brand_primary_color,
  brand_logo_url, public_website_url, public_contact_email, updated_at
) ON organizations TO camp_app;

CREATE FUNCTION get_public_catalog(requested_slug text)
RETURNS TABLE (
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog AS $$
  WITH selected_org AS (
    ...
    WHERE o.slug = requested_slug AND o.public_catalog_enabled
  ), public_sessions AS (
...
REVOKE ALL ON FUNCTION get_public_catalog(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_public_catalog(text) TO camp_app;

COMMENT ON FUNCTION get_public_catalog(text) IS 'Narrow public catalog projection. Counts and private tenant data never leave this function.';
